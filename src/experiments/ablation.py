...
class AblationStudy:
    ...
    def run_multi_config_study(
        ...
        keep_lowest: bool = False,
        sparsification_mode: str = "threshold",
        skip_configs: Optional[set] = None,
    ) -> pd.DataFrame:
        ...
        Args:
            ...
            skip_weighted: If True, skip scenarios C and D (weighted).
            keep_lowest: If True, keep lowest-scoring edges (control experiment).
            skip_configs: Optional set of (model, metric, retention, seed, scenario_value)
                         tuples to skip (used by --resume to avoid re-running existing results).
                ...
                for model_name in model_names:
                    ...
                    for seed in seeds:
                        ...
                        for scenario, exp_data, edge_weight, s_ret, topo in scenarios_config:
                            if skip_configs is not None and (
                                model_name, metric, round(retention, 6), seed, scenario.value
                            ) in skip_configs:
                                skipped_scenarios += 1
                                continue
                            result = self._run_single_experiment(
                        ...
                        if not self.results:
                            # All scenarios for this config were already done
                            continue

                        print(f"\n{'='*60}")
                        print(f"Config: {model_name} | {metric} | {retention:.0%} | Seed: {seed}")
                        if skipped_scenarios:
                            print(f"  (skipped {skipped_scenarios} already-done scenario(s))")
                        print(f"{'='*60}")
