...
def _load_done_set(output_file: Path, sparsification_type: str, experiment_type: str) -> set:
    """Return set of (model, metric, retention, seed, scenario) tuples already in the CSV."""
    if not output_file.exists():
        return set()
    try:
        df = pd.read_csv(output_file)
        required = {"SparsificationType", "ExperimentType", "Model", "Metric", "Retention", "Seed", "Scenario"}
        if not required.issubset(df.columns):
            return set()
        mask = (
            (df["SparsificationType"] == sparsification_type)
        ...
        return set(zip(
            sub["Model"],
            sub["Metric"],
            sub["Retention"].round(6),
            sub["Seed"].astype(int),
            sub["Scenario"],
        ))
    except Exception as e:
        print(f"  Warning: could not load done set from {output_file}: {e}")
        return set()


def run_single_dataset_experiments(dataset_name: str, device: str, results_dir: Path, resume: bool = False):
    """Run all experiments for a single dataset."""
    print(f"\n{'#' * 70}")
    print(f"# DATASET: {dataset_name.upper()}  |  Device: {device}")
    if resume:
        print(f"# RESUME MODE: skipping already-completed configs")
    print(f"{'#' * 70}")
    ...
    df = study.run_multi_config_study(
        ...
        skip_configs=_load_done_set(output_file, "Threshold", "threshold_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        use_metric_backbone=False,
        skip_weighted=True,
        skip_configs=_load_done_set(output_file, "Random", "threshold_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        skip_weighted=True,
        keep_lowest=True,
        skip_configs=_load_done_set(output_file, "InverseThreshold", "threshold_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        skip_configs=_load_done_set(output_file, "MetricBackbone", "backbone") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        skip_configs=_load_done_set(output_file, "Sampled", "sampled_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        skip_configs=_load_done_set(output_file, "DegreeAware", "degree_aware_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
    ...
    df = study.run_multi_config_study(
        ...
        skip_configs=_load_done_set(output_file, "Threshold", "feature_metric_sweep") if resume else None,
        **COMMON_TRAIN_KWARGS,
    )
    if not df.empty:
        df["SparsificationType"] = "Threshold"
    ...
    # ------------------------------------------------------------------
    if not all_results:
        print(f"\nNo new results for {dataset_name} (all configs already done).")
    ...
    if resume and output_file.exists():
        ...
        print(f"\nAppended {len(new_df)} new rows to {output_file} ({len(results_df)} total)")
    else:
        results_df = new_df
...
def main():
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Skip already-completed configs and append new results to existing CSV",
    )
    ...
    for dataset_name in datasets_to_run:
        try:
            run_single_dataset_experiments(dataset_name, device, RESULTS_DIR, resume=args.resume)
        except Exception as e:
