...
Estimated runtime on Kaggle T4×2:
  --skip-approxer   ~7 h    (75 configs incl. weighted variants, fits in one 12 h session)
  --approxer-only   ~8 h    (20 configs, second session after downloading CSV)
  Full run          ~15 h   (95 configs total)
...
SPARSIFICATION_CONFIGS = [
    ...
    # Weighted variants for non-ApproxER metrics (Scenario D: Sparse + Weighted)
    ('Jaccard-T-W',     'jaccard',        False, True,  'threshold'),
    ('AA-T-W',          'adamic_adar',    False, True,  'threshold'),
    ('FeatCos-T-W',     'feature_cosine', False, True,  'threshold'),
    ('Jaccard-IT-W',    'jaccard',        True,  True,  'threshold'),
    ('AA-IT-W',         'adamic_adar',    True,  True,  'threshold'),
    ('FeatCos-IT-W',    'feature_cosine', True,  True,  'threshold'),
]
...
def main():
    ...
    if args.skip_approxer:
        logger.info('Mode    : skip-approxer  (15 methods × 5 rates = 75 configs)')
