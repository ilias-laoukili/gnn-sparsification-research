...
Usage:
    python scripts/nb07_synthetic/run_mlp_control_sweep.py --h 0.1 --graph_seed 0
    python scripts/nb07_synthetic/run_mlp_control_sweep.py --h 0.5 --graph_seed 1 --resume
    python scripts/nb07_synthetic/run_mlp_control_sweep.py --h 0.1 --graph_seed 0 --arch graphsage
    python scripts/nb07_synthetic/run_mlp_control_sweep.py --h 0.1 --graph_seed 0 --arch gat
"""
...
from src.data.synthetic import build_csbm_graph
from src.hpo.config import (
    DO_MAX, DO_MIN, HIDDEN_CHOICES, LAYER_CHOICES,
    LR_MAX, LR_MIN, WD_MAX, WD_MIN,
)
from src.models.flexible import FLEX_REGISTRY, FlexibleMLP
...
# ARCH_REGISTRY — GNN architectures only (mlp handled separately)
...
def sample_hp_configs(n, seed=42):
    """Latin Hypercube sample over the HP space."""
...
def run(h, graph_seed, arch, device, resume):
    ...
    data, effective_h = build_csbm_graph(
        ...
        f=N_FEATURES, graph_seed=graph_seed, theta_exp=None, device=device,
    )
